...
**
** Wordle with color. Extra credit for project 1.
**
**
**
**
...
const string correct = "&", wrongLocation = "!", incorrect = "_",
             ...
             winningStatus = "&&&&&", red = "RED", green = "GREEN",
             yellow = "YELLOW";
...
// precondition: a string indicating color and a letter
// postcondition: prints the letter in the specified color
void printColoredChar(string color, char letter);
...
// prints a character in the specified color
void printColoredChar(string color, char letter) {
    // prints a colored letter if it is red, green, or yellow
    if (color == red) {
        cout << "\033[;31m" << letter << "\033[0m";
    } else if (color == green) {
        cout << "\033[;32m" << letter << "\033[0m";
    } else if (color == yellow) {
        cout << "\033[;33m" << letter << "\033[0m";
        // else prints it normally
    } else {
        cout << letter;
    }
}
...
void displayRecords(string gueses[], string statuses[], int numGuessed) {
    ...
    // iterates through all of the guesses
    for (int i = 0; i < numGuessed; i++) {
        // iterates through each character in the guess and status
        for (int j = 0; j < wordLength; j++) {
            // outputs each character in the guess in the color the matches the
            // corresponding status
            if (statuses[i].substr(j, 1) == correct) {
                printColoredChar(green, gueses[i][j]);
            } else if (statuses[i].substr(j, 1) == wrongLocation) {
                printColoredChar(yellow, gueses[i][j]);
            } else {
                printColoredChar(red, gueses[i][j]);
            }
        }
        cout << endl;
    }
